...
CREATE VIEW [WizardView] AS
    SELECT w.Id, w.Name AS [Name], w.OriginLevel, w.ImageName, m.Name AS [MagicType], w.DefaultMoveSetId
    FROM Wizards AS w
    INNER JOIN MagicTypes AS m ON m.Id = w.MagicTypeId;

CREATE VIEW [MoveView] AS
    SELECT mo.Id, mo.Name AS [Name], ma.Name AS [Magic Type], mo.Damage, mo.RechargeTime, mo.IsExlusive AS [IsExclusive]
    FROM Moves AS mo
    INNER JOIN MagicTypes AS ma ON ma.Id = mo.MagicTypeId;
